...
private class Invoker<EventData> {
    ...
    init<Listener: AnyObject>(listener: Listener, method: @escaping (Listener) -> (EventData) -> ()) {
...
class Event<EventData> {
    ...
    func addListener<Listener: AnyObject>(_ listener: Listener, method: @escaping (Listener) -> (EventData) -> ()) {
